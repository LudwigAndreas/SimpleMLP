...
unsigned char* load_bmp(const char* filename, int* width, int* height,
  ...
  for (int y = 0; y < height_px; ++y) {
    for (int x = 0; x < width_px; ++x) {
      infile.read((char*)&data[(height_px - y - 1) * num_channels * width_px +
                               num_channels * x],
                  num_channels);
    }
...
std::vector<float> bmp_data_to_grayscale(const unsigned char* bmp_data,
  ...
  for (int i = 0; i < width * height; i++) {
    ...
    grayscale_data[i] =
        static_cast<float>(0.2126 * r + 0.7152 * g + 0.0722 * b);
    if (grayscale_data[i] > max) max = grayscale_data[i];
    if (grayscale_data[i] < min) min = grayscale_data[i];
  }
  max = max - (max - min) / 20;
  min = min + (max - min) / 20;
