...
public:
  ...
  LetterRecognitionMLPModelBuilder *
  PerceptionBase(PerceptronBase flag) override;
  ...
  LetterRecognitionMLPModelBuilder *
  ActivationFunc(ActivationFunction::Flags flag) override;

  LetterRecognitionMLPModelBuilder *ActivationFunc(const std::string func_name);

  LetterRecognitionMLPModelBuilder *
  HiddenUnitsPerLayer(int hidden_units_per_layer) override;

  LetterRecognitionMLPModelBuilder *
  LearningRate(float learning_rate, bool use_auto_decrease) override;
