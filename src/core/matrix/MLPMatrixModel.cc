...
// 	float MLPMatrixModel::TestOutput(std::vector<s21::Sample> samples, bool
// silent_mode, std::string filename) {
// 		// std::fstream		output;
...
// std::ofstream::trunc); 		for (auto & sample : samples) {
// y_hat = Forward(sample.x); 			index =
// GetMostProbablePrediction(y_hat); 			if (index ==
// GetMostProbablePrediction(sample.y.ToVector()))
// 				++correct_guesses;
// //				if (output.is_open())
// //					log(output,
// GetMostProbablePrediction(sample.y.ToVector()), index, y_hat[index]);
// 		}
// 		accuracy = ((float)correct_guesses / samples.size());
// 		if (!silent_mode)
// 			std::cerr << "Test: " << accuracy * 100 << "% accuracy"
// << std::endl; 		return accuracy;
// 	}
