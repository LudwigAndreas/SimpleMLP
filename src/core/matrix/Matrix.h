...
public:
  ...
  Matrix multiply_elementwise(Matrix &target) {
    if (shape != target.get_shape())
      throw MatrixCalculationsException(
          "Matrix Multiplication Exception: multiply_elementwise(Matrix &). "
          "Shape is not valid");
    Matrix output((*this));
  ...
  Matrix matmul(Matrix &target) {
    if (cols != target.get_rows())
      throw MatrixCalculationsException(
          "Matrix Multiplication Exception: matmul(Matrix &). Matrix "
  ...
  Matrix matmulTransposed(Matrix &target) {
    ...
    // matmulTransposed(Matrix). Matrix dimensions not match");
  ...
  Matrix add(const Matrix &target) const {
    if (shape != target.get_shape())
      throw MatrixCalculationsException("Matrix Addition Exception: add(Matrix "
...
template <typename Type> void PrintShape(Matrix<Type> matrix) {
