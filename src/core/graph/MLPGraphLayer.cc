...
namespace s21 {
	...
	// 	this->neurons = std::move(neurons);
	// 	this->size 	  = this->neurons.size();
	// 	this->af 	  = af;
	// 	this->SetInputLayer(input);
	// 	this->SetOutputLayer(output);
	// }
	...
	MLPGraphNode	&MLPGraphLayer::operator[](int index) {
		return (neurons[index]);
	}
	...
	size_t MLPGraphLayer::InputSize() const {
		return input->size;
	}

	size_t MLPGraphLayer::Size() const {
		return size;
	}
    ...
    std::istream &operator>>(std::istream &is, MLPGraphLayer &layer) {
		...
		for (j = 0; j < input_size && std::getline(is, line); ++j) {
			...
			if (i != size)
				throw UploadFileException("Invalid weight's line length.");
		}
		if (j != input_size) {
		...
		if (std::getline(is, line)) {
			...
			if (i != size)
				throw UploadFileException("Invalid bias line length.");
		} else {
			throw UploadFileException("File ended unexpectedly.");
		}
		return is;
