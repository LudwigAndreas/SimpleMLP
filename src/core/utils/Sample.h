...
struct Sample {
    Sample() = default;
    Sample(const Matrix<float>& x_, const Matrix<float>& y_)
...
public:
    ...
    explicit Dataset(std::vector<DatasetGroup> samples)
        : groups(std::move(samples)), current_iteration(0) {}
    Dataset(const std::vector<Sample>& samples, int k) : current_iteration(0) {
        ...
        if (group.size()) groups.emplace_back(group);
    }

    DatasetGroup& operator[](size_t index) {
        return groups[index];
    }
    const DatasetGroup& operator[](size_t index) const {
        return groups[index];
    }
    ...
    bool isUsedUp() {
        return (size_t)current_iteration >= groups.size();
    }
    void resetDataset() {
        current_iteration = 0;
    }
